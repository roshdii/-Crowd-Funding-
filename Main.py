...
class Project:
	...
	def __init__(self,id,name,u_id,total,donats,start,end,tags,details,category):
	...
	def start(cls,u_id,name=None):
		...
		tmp_dic = {
			...
			'name' : name ,
			'u_id' : u_id,
			'total' : int(input("Enter total targer : ")) ,
			'donats' : 0 ,
			'st_date':input("Enter start date of campaign : "),
			'end_date':input("Enter end date of campaign : "),
	...
	@classmethod 
	def search_by_time(cls,st_time,end_time):
		print("project select ")
		# for pro in projects : 
		# 	if pro.u_id = u_id :
		# 		print(pro.name)
	...
	def donate(self):
		...
		print("Current total donation of project : ",self.total," Needed donations = ",self.donats)
		x= input("Enter donation value : ")
		try :
			if x.isdigit() :
				self.total = int(self.total) +  int(x) 
	...
	def edit(self):
		...
		# recursive update 
		print("Current name of project :",self.name)
	...
	def end(self):
		...
		# recursive update project file
		if input("Are your sure to end this project ? [Y/N]")  == 'Y' :
	...
	def view(self):
		...
		print("Name : ",self.name)
		print("User/Creator ID : ", str(self.u_id))
		print("User/Creator Name : " , User.users[int(self.u_id)-1].name )
		print("Total needed donations : ",int(self.donats))
		print("Current donats : ",int(self.total))
	...
	@classmethod
	def projects_of_time(cls,search_st_date):
		...
		# e = [(e[0]+e[1]*30),e[2]] 
		for pro in cls.projects :
			p_st = [pro.st_date[0]+ pro.st_date[1]*30 , pro.st_date[2]]
			p_en = [pro.end_date[0]+ pro.end_date[1]*30 , pro.end_date[2]]
			if s > p_st and s < p_en :
				print(pro.name) 
	...
	while True :			
		...
		if re1 == 1 :
			...
			while True :
				...
				l = Project.search_by_user(int(loged_user.id))
				clear_screen()
